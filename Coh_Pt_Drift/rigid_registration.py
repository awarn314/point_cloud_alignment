...
class RigidRegistration(EMRegistration):
    ...
    def __init__(self, R=None, t=None, s=None, ops=None, *args, **kwargs):
        ...
        if R is not None and (R.ndim is not 2 or R.shape[0] is not self.D or R.shape[1] is not self.D or not is_positive_semi_definite(R)):
            raise ValueError(
                f'The rotation matrix can only be initialized to {self.D}x{self.D} positive semi definite matrices. Instead got: {R}.')

        if t is not None and (t.ndim is not 2 or t.shape[0] is not 1 or t.shape[1] is not self.D):
            raise ValueError(
                f'The translation vector can only be initialized to 1x{self.D} positive semi definite matrices. Instead got: {t}.')

        if s is not None and (not isinstance(s, numbers.Number) or s <= 0):
            raise ValueError(
                f'The scale factor must be a positive number. Instead got: {s}.')

        self.R = np.eye(self.D) if R is None else R
        self.t = np.atleast_2d(np.zeros((1, self.D))) if t is None else t
        self.s = 1 if s is None else s
        self.ops = [True, True, True] if ops is None else ops
